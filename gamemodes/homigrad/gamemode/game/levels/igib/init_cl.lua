...
function igib.HUDPaint_RoundLeft(white)
    ...
    if startRound > 0 and lply:Alive() then
        ...
        --draw.DrawText( "" , "HomigradFontBig", ScrW() / 2, ScrH() / 2, Color( color.r,color.g,color.b,math.Clamp(startRound,0,1) * 255 ), TEXT_ALIGN_CENTER )
        draw.DrawText( "KILL KILL KILL KILL KILL KILL KILL KILL", "HomigradFontBig", ScrW() / 2, ScrH() / 1.2, Color( 55,55,55,math.Clamp(startRound,0,1) * 255 ), TEXT_ALIGN_CENTER )
        return
    end
    draw.SimpleText("Time until respawn: " .. timerr,"HomigradFont",ScrW()/2,ScrH()-55,white,TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
end