...
concommand.Add("combine_spawn", function(ply, cmd, args)
...
function basedefence.StartRoundSV()
    ...
    timer.Create("BD_npcwave", 15, 0, function()
        local plys = team.GetPlayers(1)
...
concommand.Add("all_dance", function(ply, cmd, args)
...
function basedefence.EndRound(winner)
    if winner == 1 then
        PrintMessage(3,"The combines are retreating!")

        -- make players dance/celebrate
        for _, v in ipairs(player.GetAll()) do
            if IsValid(v) then
                local newRandom = math.random(1, 4)

                if newRandom == 1 then
                    v:ConCommand("act dance")
                elseif newRandom == 2 then
                    v:ConCommand("act cheer")
                elseif newRandom == 3 then
                    v:ConCommand("act robot")
                elseif newRandom == 4 then
                    v:ConCommand("act muscle")
                end
            end
        end
    else
        PrintMessage(3,"The combines neutralized the rebels!")
    end
...
function basedefence.PlayerSpawn2(ply,teamID)
    if teamID == 2 then ply:ChatPrint("Returning...") return end
...
function basedefence.PlayerCanJoinTeam(ply,teamID)
	if teamID == 3 then ply:ChatPrint("Not Available.") return false end
    if teamID == 2 then
        ply:ChatPrint("Not Available.")
